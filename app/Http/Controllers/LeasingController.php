...
class LeasingController extends Controller
{
    ...
    public function actionGuardarLeasingCuentaContable($idopcion, Request $request)
    {

        $asientos = json_decode($request['asientos_leasing'], false);
        foreach ($asientos as $index_asiento => $asiento) {
            ...
            foreach ($cabecera as $index => $item) {
                ...
                $COD_ASIENTO_MODELO = '';
                $TXT_TIPO_REFERENCIA = 'PROVISION_LEASING'; // $item->tipo_referencia;
                ...
                $anular_asiento = $this->leasing_anular_asiento($asiento_id,
        ...
        return Redirect::to('/gestion-pago-leasing/' . $idopcion)->with('bienhecho', 'Registro cuenta contable exitoso');

    }

    public function actionAjaxModalConfiguracionLeasingCuentaContable(Request $request)
    {
        ...
        foreach ($request['datastring'] as $registro) {
            ...
            $monto_total = $this->leasing_monto_total_asiento($lista_leasing, $registro['cod_pagare'], $registro['cod_detalle_pagare']);
            $glosa = 'PAGO DE OBLIGACIONES: LEASING INTERESES / '; //. $registro['nombre_banco']
            $tipo_referencia = 'PROVISION_LEASING';
        ...
        $cabecera = $this->leasing_cabecera_asiento($periodo, $empresa_id, $montos_totales, $glosa, $moneda_id, $moneda, $tipo_cambio, $tipo_referencia);
        $asientos_leasing[0]['cabecera'] = $cabecera;
        $asientos_leasing[0]['detalle'] = $detalles;

        $funcion = $this;

        return View::make('leasing/modal/ajax/amleasingcc',
            [
                'asientos_leasing' => $asientos_leasing,
                'periodo' => $periodo,
                'funcion' => $funcion,
                'idopcion' => $idopcion,
                'ajax' => true,
    ...
    public function actionListarLeasing($idopcion)
    {
        $validarurl = $this->funciones->getUrl($idopcion, 'Ver');
        if ($validarurl != 'true') {return $validarurl;}

        View::share('titulo', 'Lista de Leasings');
        ...
        $lista_leasing = $this->lista_leasing($empresa_id, $sel_periodo, '');
        ...
        return View::make('leasing/listaleasing',
            [
                'lista_leasing' => $lista_leasing,
    ...
    public function actionAjaxRegistroLeasing(Request $request)
    {
        ...
        $lista_leasing = $this->lista_leasing($empresa_id, $periodo_id, '', $banco_id);

        $funcion = $this;

        return View::make('leasing/ajax/alistaleasing',
            [
                'lista_leasing' => $lista_leasing,
                'funcion' => $funcion,
    ...
    public function actionDetalleLeasing($cod_pagare)
    {

        $lista_detalle_leasing = $this->lista_detalle_leasing($cod_pagare);

        return View::make('leasing/detalleleasing', ['lista_detalle_leasing' => $lista_detalle_leasing]);
    }
}
