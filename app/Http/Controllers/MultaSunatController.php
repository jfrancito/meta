...
use App\Traits\MultaSunatTraits;
...
class MultaSunatController extends Controller
{
    //
    use GeneralesTraits;
    use AsientoModeloTraits;
    use PlanContableTraits;
    use MultaSunatTraits;

    public function actionGuardarMultaSunatCuentaContable($idopcion, Request $request)
    {

        $asientos = json_decode($request['asientos_multa_sunat'], false);
        foreach ($asientos as $index_asiento => $asiento) {
            ...
            foreach ($cabecera as $index => $item) {
                ...
                $COD_CATEGORIA_TIPO_ASIENTO = 'TAS0000000000007'; //'TAS0000000000007';
                $TXT_CATEGORIA_TIPO_ASIENTO = 'DIARIO'; //'DIARIO';
                $NRO_ASIENTO = '';
                ...
                $COD_ASIENTO_MODELO = '';
                $TXT_TIPO_REFERENCIA = 'MULTA_SUNAT'; // $item->tipo_referencia;
                ...
                $anular_asiento = $this->multa_sunat_anular_asiento($asiento_id,
        ...
        return Redirect::to('/gestion-pago-multa-sunat/' . $idopcion)->with('bienhecho', 'Registro cuenta contable exitoso');

    }

    public function actionAjaxModalConfiguracionMultaSunatCuentaContable(Request $request)
    {
        ...
        $asientos_multa_sunat = array();

        foreach ($request['datastring'] as $registro) {

            $monto_total = $monto_total + $this->multa_sunat_monto_total_asiento($lista_multa_sunat, $registro['documento_id']);

            $glosa = 'EGRESO DIRECTO POR: PAGO DE PAGO DE MULTA' . ' / ' . $registro['nombre_banco'];
            $tipo_referencia = 'MULTA_SUNAT';
        }
        ...
        $asientos_multa_sunat[0]['cabecera'] = $cabecera;
        $asientos_multa_sunat[0]['detalle'] = $detalle;

        $funcion = $this;

        return View::make('multasunat/modal/ajax/ammultasunatcc',
            [
                'asientos_multa_sunat' => $asientos_multa_sunat,
                'periodo' => $periodo,
                'funcion' => $funcion,
                'idopcion' => $idopcion,
                'ajax' => true,
    ...
    public function actionMultaSunatGuardarData(Request $request)
    {
        ...
        $glosa = 'EGRESO DIRECTO POR PAGO DE MULTA';
        $descripcion = 'CC_PAGO_MULTA';
    ...
    public function actionListarMultaSunat($idopcion)
    {
        $validarurl = $this->funciones->getUrl($idopcion, 'Ver');
        if ($validarurl != 'true') {return $validarurl;}

        View::share('titulo', 'Lista de Multas');
        ...
        return View::make('multasunat/listamultasunat',
            [
                'lista_multa_sunat' => $lista_multa_sunat,
    ...
    public function actionAjaxRegistroMultaSunat(Request $request)
    {
        ...
        return View::make('multasunat/ajax/alistamultasunat',
            [
                'lista_multa_sunat' => $lista_multa_sunat,
                'funcion' => $funcion,
