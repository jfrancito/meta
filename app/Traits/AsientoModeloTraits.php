...
trait AsientoModeloTraits
{
	...
	private function am_asiento_modelo_efectivo_detalle($cod_categoria_tipo_documento,$buscar_modelo_asiento)
	{
		$asientomodelodetalle 				= 	'';
		if($buscar_modelo_asiento['encontro'] == '1'){

			//dd($cod_categoria_tipo_documento);
			...
			if(count($listaasientomodelodetalle)>0){
				...
				$asientomodelodetalle  =    WEBAsientoModeloDetalle::join('WEB.cuentacontables', 'WEB.cuentacontables.id', '=', 'WEB.asientomodelodetalles.cuenta_contable_id')
											...
											->whereNotIn('WEB.cuentacontables.nro_cuenta', ['102101','102102'])
	...
	private function am_asiento_detalle_efectivo_array($movimiento,$tipo_asiento,$anio,$pago_cobro,$empresa_rel_ter,$buscar_modelo_asiento,$nro_operacion,$nro_referencia,$listadetallemovimientos,$funcion,$existe_rel_ter,$abreviatura_asiento_modelo)
	{
		$array_detalle_asiento  		= 	array();
		...
		if($buscar_modelo_asiento['encontro'] == '1'){
			...
			foreach($listaasientomodelodetalle as $index => $item){
				...
				if($item->cuentacontable->nro_cuenta == '102101' || $item->cuentacontable->nro_cuenta == '102102' ){
					$debe_haber  = $this->am_debe_haber($item->partida_id);
					$DEBE_MN=0;
					$HABER_MN=0;
					$DEBE_ME=0;
					...
					if($debe_haber == 'DEBE'){
			            if($nro_referencia == 'IACHAN0000000005'){
			            	if($movimiento->TXT_CATEGORIA_MONEDA == 'DOLARES'){
				            	$DEBE_MN=$movimiento->CAN_HABER_ME*$tipo_cambio_cp->CAN_VENTA_SBS;
								$DEBE_ME=$movimiento->CAN_HABER_ME; 
			            	}else{
				    ...
				    $cuentacontable 	= 	WEBCuentaContable::where('empresa_id','=',Session::get('empresas_meta')->COD_EMPR)
											->where('anio','=',$anio)
											->where('id','=',$item->cuentacontable->id)
											->where('activo','=',1)
											->first();
					$COD_CUENTA_CONTABLE='';
					$TXT_CUENTA_CONTABLE='';
			...
			foreach($listadetallemovimientos as $index => $item){
				...
				$asientomodelodetalle     	= 	$this->am_asiento_modelo_efectivo_detalle($item->COD_CATEGORIA_TIPO_DOC,$buscar_modelo_asiento);
	...
	private function am_asiento_efectivo_array($movimiento,$tipo_asiento,$anio,$pago_cobro,$empresa_rel_ter,$buscar_modelo_asiento,$nro_operacion,$nro_referencia,$glosa,$text_pago_cobro,$periodo)
	{
	...
	private function am_buscar_asiento_modelo_efectivo($movimiento,$tipo_asiento,$anio,$pago_cobro,$empresa_rel_ter)
	{
