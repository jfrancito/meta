...
use App\Modelos\WEBViewMigrarReciboHonorario;
...
trait MigrarReciboHonorarioTraits
{
	
	private function mrh_lista_compras_migrar_agrupado_emitido($array_empresas,$anio)
	{
		...
		$lista_migrar_compras		=		WEBViewMigrarReciboHonorario::leftJoin('WEB.historialmigrar', function ($join) {
									            $join->on('WEB.historialmigrar.COD_REFERENCIA', '=', 'WEB.viewmigrarrecibohonorario.COD_DOCUMENTO_CTBLE')
									                 ->where('WEB.historialmigrar.IND_ASIENTO_MODELO', '=', 1);
									        })
											->whereNull('WEB.historialmigrar.COD_REFERENCIA')
											->whereIn('WEB.viewmigrarrecibohonorario.COD_PERIODO',$array_periodo)
											->whereIn('WEB.viewmigrarrecibohonorario.COD_EMPR',$array_empresas)
											//->where('WEB.viewmigrarrecibohonorario.COD_DOCUMENTO_CTBLE','=','ICCHFC0000047269')
											->where('WEB.viewmigrarrecibohonorario.NOM_ESTADO','=','ENVIADO')
											->select(DB::raw('WEB.viewmigrarrecibohonorario.COD_DOCUMENTO_CTBLE'))
											->groupBy('WEB.viewmigrarrecibohonorario.COD_DOCUMENTO_CTBLE')
											->get();
	...
	private function mrh_agregar_historial_compras($lista_compras_migrar_emitido,$lista_compras_migrar_anulado,$tipo_asiento)
	{
		...
		foreach($lista_compras_migrar_emitido as $index => $item){
			...
			$historialmigrar 							=   WEBHistorialMigrar::where('COD_REFERENCIA','=',$item->COD_DOCUMENTO_CTBLE)
															...
															->where('TXT_REFERENCIA','=','RECIBO_HONORARIO')
															->first();
			...
			if(count($historialmigrar)<=0){
				...
				$cabecera->TXT_CATEGORIA_TIPO_ASIENTO 	=   'DIARIO';
				...
				$cabecera->TXT_REFERENCIA 				=   'RECIBO_HONORARIO';
	...
	private function mrh_update_historial_compras($documento_ctble_cod,$tipo_asiento)
	{
        ...
        $stmt 						= 		DB::connection('sqlsrv')->getPdo()->prepare('SET NOCOUNT ON;EXEC WEB.BUSCAR_ASIENTO_MODELO_RH 
											@anio = ?,
											@empresa = ?,
											@cod_contable = ?,
											@cod_tipo_asiento = ?,
	...
	private function mrh_lista_compras_asignar($array_empresas,$tipo_asiento)
	{
	...
	private function mrh_asignar_asiento_modelo($historialmigrar,$tipo_asiento)
	{
        ...
        $stmt 						= 		DB::connection('sqlsrv')->getPdo()->prepare('SET NOCOUNT ON;EXEC WEB.APLICAR_ASIENTO_MODELO_RH 
											@anio = ?,
											@empresa = ?,
											@cod_contable = ?,
											@cod_tipo_asiento = ?,
