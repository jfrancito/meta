...
trait MigrarVentaInternacionalTraits
{
	public function mv_array_empresa_venta_internacional(){
        $array_empresas  		    = 		['IACHEM0000010394'];
        return $array_empresas;
    }
	
	private function mvi_lista_ventas_migrar_agrupado_emitido_internacional()
	{
		
		$array_empresas  		    = 		$this->mv_array_empresa_venta_internacional();
	...
	private function mvi_lista_ventas_migrar_agrupado_anulado_internacional()
	{
	...
	private function mv_agregar_historial_ventas_internacional($lista_ventas_migrar_emitida,$lista_ventas_migrar_anulada,$tipo_asiento)
	{
	...
	private function mv_update_historial_ventas_internacional($documento_ctble_cod,$tipo_asiento)
	{
		...
		//enviara ivap o enviar a igv
		//IVAP
		$sw 						=		0;
		$producto 					= 		CMPDetalleProducto::where('COD_TABLA','=',$documento_ctble_cod)
											->get();

		$ivap 						= 		CMPDetalleProducto::join('WEB.productoempresas', 'WEB.productoempresas.producto_id', '=', 'CMP.DETALLE_PRODUCTO.COD_PRODUCTO')
											->where('CMP.DETALLE_PRODUCTO.COD_TABLA','=',$documento_ctble_cod)
											->where('WEB.productoempresas.empresa_id','=',$documento_ctble->COD_EMPR)
											->where('WEB.productoempresas.anio','=',$anio)
											->where('WEB.productoempresas.cuenta_contable_venta_relacionada_id','<>','')
											->get();

		$igv 						= 		CMPDetalleProducto::join('WEB.productoempresas', 'WEB.productoempresas.producto_id', '=', 'CMP.DETALLE_PRODUCTO.COD_PRODUCTO')
											->where('CMP.DETALLE_PRODUCTO.COD_TABLA','=',$documento_ctble_cod)
											->where('WEB.productoempresas.empresa_id','=',$documento_ctble->COD_EMPR)
											->where('WEB.productoempresas.anio','=',$anio)
											->where('WEB.productoempresas.cuenta_contable_venta_aigv_relacionada_id','<>','')
											->get();

		if(count($ivap)>0 && count($igv)>0){
			$sw 						=		1;			
		}
		...
		if($sw == 0){
			...
			if(count($ivap)>0){
		        ...
		        $stmt 						= 		DB::connection('sqlsrv')->getPdo()->prepare('SET NOCOUNT ON;EXEC WEB.BUSCAR_ASIENTO_MODELO_INTERNACIONAL 
													@anio = ?,
													@empresa = ?,
													@cod_contable = ?,
													@cod_tipo_asiento = ?,
	...
	private function mv_lista_ventas_asignar_internacional($tipo_asiento)
	{
	...
	private function mv_asignar_asiento_modelo_internacional($historialmigrar,$tipo_asiento)
	{
