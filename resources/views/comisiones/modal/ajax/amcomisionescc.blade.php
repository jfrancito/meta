...
<div class="modal-body">
	<div class="scroll_text scroll_text_heigth_aler" style = "padding: 0px !important;"> 
        ...
        <div class="panel-group" id="accordion">
            @foreach($asientos_comisiones as $index_asiento => $asiento)
            <div class="panel panel-default">
              <div class="panel-heading">
                <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{$index_asiento}}">
                            {{$asientos_comisiones[$index_asiento]['cabecera'][0]['glosa']}}  -  {{$asientos_comisiones[$index_asiento]['cabecera'][0]['moneda'] == 'SOLES' ? 'S/. ' : '$ '}} {{number_format($asientos_comisiones[$index_asiento]['cabecera'][0]['total_debe'], 2, '.', ',')}}
                    </a>
                </h4>
              </div>
	...
	<form method="POST"
	id="formguardar"
	action="{{ url('/comisiones-guardar-data/'.$idopcion) }}" 
...
{{-- 		<input type="hidden" name="cabecera" id='cabecera' value='{{json_encode($asiento['cabecera'])}}'>
        ...
        <input type="hidden" name="asientos_comisiones" id='asientos_comisiones' value='{{json_encode($asientos_comisiones)}}'>
